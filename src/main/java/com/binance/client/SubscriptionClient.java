...
/***
 * The subscription client interface, it is used for subscribing any market data update and account change, it is
 * asynchronous, so you must implement the SubscriptionListener interface. The server will push any update to the
...
public interface SubscriptionClient
{
	...
	/**
	 * Subscribe aggregate trade event. If the aggregate trade is updated, server will send the data to client and onReceive
	...
	/**
	 * Subscribe mark price event. If the mark price is updated, server will send the data to client and onReceive in
	...
	/**
	 * Subscribe individual symbol ticker event. If the symbol ticker is updated, server will send the data to client and
	 * onReceive in callback will be called.
	 *
	 * @param symbol       The symbol, like "btcusdt".
	 * @param callback     The implementation is required. onReceive will be called if receive server's update.
	...
	/**
	 * Subscribe all market tickers event. If the tickers are updated, server will send the data to client and onReceive in
	 * callback will be called.
	 *
	 * @param callback     The implementation is required. onReceive will be called if receive server's update.
	 * @param errorHandler The error handler will be called if subscription failed or error happen between client and
	...
	/**
	 * Subscribe partial book depth event. If the book depth is updated, server will send the data to client and onReceive
	 * in callback will be called.
	 *
	 * @param symbol       The symbol, like "btcusdt".
	 ...
	 *                     Binance server.
	 */
	void subscribeBookDepthEvent(String symbol, Integer limit, SubscriptionListener<OrderBookEvent> callback, SubscriptionErrorHandler errorHandler);
	...
	/**
	 * Subscribe user data event. If the user data is updated, server will send the data to client and onReceive in callback
	 * will be called.
	 *
