...
abstract class RestApiInvoker
{
	...
	static void checkResponse(JsonWrapper json)
	{
		try
		{
			if (json.containKey("success"))
			{
				...
				if (!success)
				{
					...
					{
						throw new BinanceApiException(BinanceApiException.EXEC_ERROR, "[Executing] " + err_code + ": " + err_msg);
					}
				}
			}
		...
		{
			throw new BinanceApiException(BinanceApiException.RUNTIME_ERROR, "[Invoking] Unexpected error: " + e.getMessage());
		}
	}
